...
class Match < ActiveRecord::Base
  ...
  scope :by_player_and_date_week, ->(player, date) { where("(player_1_id = ? OR player_2_id = ?) AND (date >= ? AND date <= ?)", player.id, player.id, date.beginning_of_week, date.end_of_week)}
